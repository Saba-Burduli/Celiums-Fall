...
local function planRoute(nav, runtime, current, goal)
  local goalChanged = nav.goalNode ~= goal.id
  nav.route = Graph.route(runtime, current.id, goal.id)
  nav.waypoint = 1
  nav.goalNode = goal.id
  nav.replanTimer = Config.navigation.replanInterval
  if goalChanged then
    nav.waitTime, nav.waitState = 0, nil
  end
end

local function moveAtGoal(e, player, current, preferredDistance, dt)
  local targetX = player.x
  if preferredDistance then
    local dx = player.x - e.x
    if math.abs(dx) < preferredDistance - 35 then
      targetX = e.x - (dx >= 0 and 1 or -1) * 80
    elseif math.abs(dx) <= preferredDistance + 35 then
      targetX = e.x
    end
  end
...
function Navigation.update(e, player, runtime, dt, preferredDistance)
  ...
  if nav.replanTimer == 0 or nav.goalNode ~= goal.id or not nav.route then
    planRoute(nav, runtime, current, goal)
  end
  if not nav.route then return true end
  while nav.waypoint < #nav.route and nav.route[nav.waypoint] == current.id do
    nav.waypoint = nav.waypoint + 1
  end
  if current.id == goal.id or nav.waypoint > #nav.route then
    moveAtGoal(e, player, current, preferredDistance, dt)
    return true
  end
  local nextId = nav.route[nav.waypoint]
  local target = graph.byId[nextId]
  local edge = Graph.edgeTo(graph, current.id, nextId)
  if not target or not edge then
    nav.route, nav.replanTimer = nil, 0
    return true
  end
  ...
  elseif edge.kind == "drop" then
    ...
    safeMove(e, current, targetX, e.speed, dt)
    if math.abs(e.x - targetX) < 10 and Collision.dropThrough(e) then
      nav.waitState = "dropping"
    end
  else
